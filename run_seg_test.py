...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                if retina_masks:
                    # scalewwwwwwwwww bbox first the crop masks
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    if save_txt:  # Write to file
                        if segments[j].size > 0:  # Ensure mask/segment is present
                            seg = segments[j].reshape(-1)  # (n,2) to (n*2)
                            # Convert normalized coordinates to original
                            original_seg = [int(x * im0.shape[1]) if i % 2 == 0 else int(x * im0.shape[0]) for i, x in enumerate(seg)]
                            line = (names[int(cls)], *original_seg, f"{conf.item():.2f}")  # convert tensor to float
